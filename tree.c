    ...
    while (i < count) {
        ...
        if (slash == NULL) {
            ...
            while (j < count) {
                ...
                if (strlen(prefix) > 0) p = p + strlen(prefix) + 1;
                if (strncmp(p, subdir, subdir_len) == 0 && p[subdir_len] == '/')
                    j++;
                else
                    break;
            }
...
int tree_from_index(ObjectID *id_out) {
    // Read index file directly (no index_load dependency)
